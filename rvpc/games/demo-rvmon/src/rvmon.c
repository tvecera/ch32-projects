...
uint8_t is_hex_digit(const char letter) {
...
uint8_t parse() {
    ...
    const uint8_t cmd_len = strlen(mon_cmdline);
    const uint8_t start_pos = cmd_pos;
    ...
    while (cmd_pos < cmd_len) {
        if (is_hex_digit(mon_cmdline[cmd_pos])) {
            const uint8_t hex_digit = (mon_cmdline[cmd_pos] <= '9')
...
void execute() {
    mon_cmd = 'R';
    if (mon_cmdline[0] == '\0') {
        snprintf(mon_cmdline, VGA_NUM_COLS, "%lX", mon_address[0]);
    ...
    switch (letter) {
        case 0x08:
            if (cmd_len > 0) {
                ...
                const uint8_t col = vga_cursor_col() - 1;
...
void app_run() {
    ...
    if (vga_cursor_col() < VGA_NUM_COLS - 1) {
        const char cursor = SysTick->CNT & (1 << 24) ? '_' : ' ';
        PrintCharAt(cursor, vga_cursor_col(), vga_cursor_row(), COL_WHITE);
    }

    const uint16_t key_code = KeyGetRaw();
        ...
        switch (key_code) {
            ...
            default:
                ...
                if ((key_code & 0xFF00) == 0xF000) {
                    const uint8_t key_char = kbd_to_ascii(key_code);
