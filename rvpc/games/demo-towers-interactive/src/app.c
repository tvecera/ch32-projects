...
void splash() {
	...
	vga_print_at(row++, 1, "VGA by Curtis Whitley");
	row += 2;
	vga_print_at(row,   5, "Hanoi Towers");
	row += 4;
	vga_print_at(row++, 1, "A,B,C Set FROM and TO");
	vga_print_at(row++, 1, "Enter Make the move");
	vga_print_at(row++, 1, "Esc   Reset the game");
...
void print_status() {
	...
	if (app_pause) {
		...
		const u8 len = DecUNum(DecNumBuf, current_move, 0);
		vga_clear_rect(STATUS_ROW, 0, STATUS_ROW, VGA_NUM_COLS-1);
		vga_print_at(STATUS_ROW, 1, "SUCCESS in ");
		vga_print_at(STATUS_ROW, 12, DecNumBuf);
		vga_print_at(STATUS_ROW, 12 + len, " moves");
		current_move = 0;
		delay = STARTUP_DELAY;
	} else {
		current_move++;
		if (current_move < 100) {
			vga_print_at(STATUS_ROW, 1, "Move #   from   to   ");
			DecUNum(DecNumBuf, current_move, 0);
			vga_print_at(STATUS_ROW, MOVE_COL, DecNumBuf);
			vga_cursor_set(STATUS_ROW, vga_cursor_col() > VGA_NUM_COLS ? FROM_COL : vga_cursor_col());
		} else {
