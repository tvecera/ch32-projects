...
#if VMODE > 5 && USE_RCA
#if USE_CUSTOM_FONT
    const font_desc_t* DrawFont = NULL;		// current draw font (characters 8x8)
#else
	const font_desc_t* DrawFont = &FONT;		// current draw font (characters 8x8)
#endif
#else
...
#elif (VMODE > 5) // text modes

	memmove(&FrameBuf[0], &FrameBuf[WIDTHBYTE], FRAMESIZE-WIDTHBYTE);
	memset(&FrameBuf[FRAMESIZE-WIDTHBYTE], 0, WIDTHBYTE);
...
#if (VMODE > 5) // text modes

	// save character
	u8* dst = &FrameBuf[WIDTHBYTE*y + x];
#if USE_RCA
	*dst = MapFont(DrawFont, ch);
#else
	*dst = ch;
#endif
