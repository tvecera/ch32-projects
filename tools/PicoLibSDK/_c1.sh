...
source $(dirname "$0")/../../sdk/PicoLibSDK/_setup.sh $1
...
if [[ "${TARGET}" == "LOADER" ]]; then
  ...
  if [[ ! -e $(dirname "$0")/../../sdk/PicoLibSDK/"${DEVDIR}"/"${GRPDIR}"/*.UF2 ]]; then mkdir $(dirname "$0")/../../sdk/PicoLibSDK/"${DEVDIR}"/"${GRPDIR}"; fi
  cp "${TARGET}".uf2 $(dirname "$0")/../../sdk/PicoLibSDK/"${DEVDIR}"/"${GRPDIR}"/"${TARGET}".UF2 > null
  exit 0
fi
...
# Detect operating system and architecture for LoaderCrc
OS="$(uname -s)"
ARCH="$(uname -m)"
...
case "$OS" in
    Darwin*)
        case "$ARCH" in
            arm64|aarch64)
                LOADERCRC="$(dirname "$0")/_tools/aarch64-macos/LoaderCrc"
                ;;
            x86_64)
                LOADERCRC="$(dirname "$0")/_tools/x86_64-macos/LoaderCrc"
                ;;
            *)
    ...
    Linux*)
        case "$ARCH" in
            aarch64)
                LOADERCRC="$(dirname "$0")/_tools/aarch64-linux-gnu/LoaderCrc"
                ;;
            x86_64)
                LOADERCRC="$(dirname "$0")/_tools/x86_64-linux-gnu/LoaderCrc"
                ;;
            *)
                echo "Unsupported architecture: $ARCH"
                exit 1
...
if [ ! -x "$LOADERCRC" ]; then
    echo "Error: LoaderCrc not found at $LOADERCRC"
    exit 1
fi

"$LOADERCRC" "${TARGET}".bin "${TARGET}".uf2
