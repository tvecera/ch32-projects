 ...
 * Uses Timer 1 channel 4 for PWM generation.
 *
 * Based on CH32LibSDK by Miroslav Nemecek (Panda38)
 * https://github.com/Panda381/CH32LibSDK
 *
 * Hardware:
 * - PC4: Audio output (TIM1_CH4, remap 0)
 *
 * Features:
 * - USE_SOUND=1: Single tone playback
 * - USE_SOUND=2: Melody playback with note sequences
 *
 * ============================================================================
 */
...
/**
 * @brief Check if sound/melody is currently playing
...
/**
 * @brief Initialize sound output
 *
 * Configures TIM1_CH4 for PWM output on PC4.
 * Timer runs at 1 MHz time base for note frequency generation.
 */
void SoundInit();

/**
 * @brief Terminate sound output
 *
 * Stops any playing sound and resets the timer.
 */
void SoundTerm();

/**
 * @brief Play a single tone
 * @param div Timer divider value (use SND_GET_DIV(hz01) or NOTE_* constants)
 *
 * Frequency = 1MHz / div. Use SND_GET_DIV(hz01) macro for custom frequencies
 * where hz01 is frequency in 0.01 Hz units (e.g., 44000 = 440.00 Hz = A4).
 */
void PlayTone(u32 div);

/**
 * @brief Stop playing tone or melody
 */
void StopSound();

#if USE_SOUND == 2
/**
 * @brief Update melody playback state
 *
 * Should be called periodically (from frame callback).
 * Advances to next note when current note duration expires.
 */
void SoundScan();

/**
 * @brief Start playing a melody
 * @param melody Pointer to array of sMelodyNote (terminated with len=0)
 */
